...
interface ICreatedRepos {
    yearLabel: string
    reposCreated: number
}
interface ITotalContributions {
    yearLabel: string
...
export const getters: GetterTree<StatisticsState, StatisticsState> = {
    ...
    totalPullRequestReviewContributions: (state) =>
        state.totalPullRequestReviewContributions,
    totalRepositoriesWithContributedCommits: (state) =>
        state.totalRepositoriesWithContributedCommits,
    totalRepositoriesWithContributedIssues: (state) =>
        state.totalRepositoriesWithContributedIssues,
...
export const mutations: MutationTree<StatisticsState> = {
    ...
    ADD_TOTAL_CONTRIBUTIONS_BY_YEAR: (
        state: StatisticsState,
        data: ITotalContributions
...
const getWeeklyContributions = (week: any) => {
    ...
    const weekContributions = weekContributionsArray.reduce(
        (a: any, b: any) => a + b,
        0
    )
...
const sanitazeData = (contributionCalendarWeeks: any) => {
    ...
    const montlyContributions = montlyContributionsArray.map((item: any) =>
        item.reduce((a: any, b: any) => a + b)
    )
...
export const actions: ActionTree<StatisticsState, StatisticsState> = {
    async fetchData({ commit }) {
        ...
        for (let year of years) {
            ...
            commit(
                'ADD_REPOS_PR_REVIEWS',
                totalRepositoriesWithContributedPullRequestReviews
            )
            commit('ADD_CREATED_REPOS_BY_YEAR', {
                yearLabel: year.label,
                reposCreated: totalRepositoryContributions,
            })
            commit(
                'ADD_TOTAL_CONTRIBUTIONS',
                contributionCalendar.totalContributions
            )
            commit('ADD_TOTAL_CONTRIBUTIONS_BY_YEAR', {
                yearLabel: year.label,
                totalContributions: contributionCalendar.totalContributions,
            })
            const sanitazedData = {
                data: await sanitazeData(contributionCalendarWeeks),
                label: await sanitazeLabels(
